...
Object.assign(mcs, {
	init: function () {
		...
		this.scriptServer.on('console', line => {
			...
			if (result) {
				status.players = []; // clear player list on startup
			...
			if (result) {
				...
				status.players.push({ name: plyr, date: new Date() });
			...
			if (result) {
				let plyr = result[1];
				for (let i = status.players.length; i >= 0; --i) {
					if (status.player[i].name === plyr) {
						status.players.splice(i, 1); // remove the player if found
					}
				}
				this.emit('status', this.status);
			}
			...
			if (result) {
				status.players = []; // clear player list on shutdown
