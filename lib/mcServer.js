		...
		this.scriptServer.on('console', line => {
			...
			if (result) {
				...
				if (this.scriptServer.rcon.state != 'connected') {
					...
					this.emit('log', 'rcon not connected');
					//this.scriptServer.rcon.connect(); //TODO make sure the rcon reconnects
				}
			}
			...
			if (result) {
				let plyr = result[1];
				for (let i = this.status.players.length - 1; i >= 0; --i) {
					if (this.status.players[i].name === plyr) {
